...
def pso(settings):
    ...
    optimizer = ps.single.LocalBestPSO(n_particles=20, dimensions=len(lbounds), options=options, bounds=bounds)

    # Perform optimization
    if settings["dva"] == 'True':
        cost, pos = optimizer.optimize(fun.obj_fun_dva, iters=15, **exp_dict)
    else:
        cost, pos = optimizer.optimize(fun.obj_fun, iters=15, **exp_dict)
